...
- **`news-watch-routine` is live**: daily headlines at 07:00 UTC (all regions/categories), Monday = weekly synthesized deep-dive instead — same day-of-week mode switch tech-watch-routine uses. Live-clones `news-watch-config`, runs in sequential waves of ≤5 (region, category) pairs to respect the shared WebSearch budget across all ~45 combinations, and authenticates to Discord via a **webhook** rather than an embedded bot token — Claude Code's safety classifier specifically refused embedding a Discord bot token directly into a routine's stored prompt when this was tried for culture-watch-routine (three attempts, three different ways); a webhook sidesteps that entirely and is what all three `*-watch-routine`s now use. Also fireable on-demand via `#news-cli`'s `run` command.
